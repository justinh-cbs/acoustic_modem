...
if 'Slant Range (m)' in data.columns:
    ax1.plot(data['Timestamp'], data['Slant Range (m)'], '-', color='blue', label='Slant Range')
    
# horizontal distance
if 'Horizontal Dist (m)' in data.columns:
    ax1.plot(data['Timestamp'], data['Horizontal Dist (m)'], '-', color='green', label='Horizontal Distance')
...
if 'Signal Quality' in data.columns:
    ax2.plot(data['Timestamp'], data['Signal Quality'], '-', color='red', label='Signal Quality')
    ax2.legend()
...
if 'Value' in data.columns:
    ax3.plot(data['Timestamp'], data['Value'], '-', color='magenta', label='Temperature')
    ax3.legend()
...
if 'Velocity (m/s)' in data.columns:
    ax4.plot(data['Timestamp'], data['Velocity (m/s)'], '-', color='cyan', label='Velocity')
