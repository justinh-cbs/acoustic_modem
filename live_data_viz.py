...
class UwaveRealtimeMonitor:
    ...
    def setup_plots(self):
        ...
        self.timestamp_text = self.fig.text(0.5, 0.01, '', 
        ...
        try:
            if os.path.exists(self.csv_file):
                ...
                if (self.last_modified is None or 
                    ...
                    print(f"Data file changed, reloading... (Size: {current_size} bytes)")
    ...
    def update_plot(self, frame):
        ...
        if 'Slant Range (m)' in self.data.columns:
            self.axes['distance'].plot(self.data['Timestamp'], self.data['Slant Range (m)'], 
                                      '-o', color='blue', label='Slant Range', markersize=4)
        
        if 'Horizontal Dist (m)' in self.data.columns:
            self.axes['distance'].plot(self.data['Timestamp'], self.data['Horizontal Dist (m)'], 
                                      '-s', color='green', label='Horizontal Distance', markersize=4)

        self.axes['distance'].legend()
        self.axes['distance'].set_xlim(time_min, time_max)

        if 'Signal Quality' in self.data.columns:
            self.axes['signal'].plot(self.data['Timestamp'], self.data['Signal Quality'], 
                                    '-', color='red', label='Signal Quality')
        ...
        if 'Value' in self.data.columns:
            self.axes['temp'].plot(self.data['Timestamp'], self.data['Value'], 
                                  '-', color='magenta', label='Temperature')
        ...
        if 'Velocity (m/s)' in self.data.columns:
            self.axes['velocity'].plot(self.data['Timestamp'], self.data['Velocity (m/s)'], 
                                      '-', color='cyan', label='Velocity')
            ...
            vel_max = max(abs(self.data['Velocity (m/s)'].max()), 
        ...
        self.timestamp_text.set_text(f'Last Updated: {current_time} | Displaying {len(self.data)} points')
...
def main():
    ...
    if custom:
        ...
        try:
            update_interval = int(input(f"Update interval in milliseconds (default: {update_interval}): ") or update_interval)
        except ValueError:
            print(f"Invalid value, using default: {update_interval}")
            
        try:
            max_points = int(input(f"Maximum points to display (default: {max_points}): ") or max_points)
        except ValueError:
